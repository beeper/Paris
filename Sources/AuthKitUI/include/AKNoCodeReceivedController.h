#import <Foundation/Foundation.h>
//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 15 2018 10:31:50).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <AppKit/NSViewController.h>

@class NSButton, NSTextField;

@interface AKNoCodeReceivedController : NSViewController
{
    NSTextField *_titleTextField;
    NSTextField *_messageTextField;
    NSButton *_cancelButton;
    NSButton *_alternativeButton;
    NSButton *_defaultButton;
    id _cancelButtonAction;
    id _alternateButtonAction;
    id _defaultButtonAction;
}


@property(copy) id defaultButtonAction; // @synthesize defaultButtonAction=_defaultButtonAction;
@property(copy) id alternateButtonAction; // @synthesize alternateButtonAction=_alternateButtonAction;
@property(copy) id cancelButtonAction; // @synthesize cancelButtonAction=_cancelButtonAction;
@property(retain) NSButton *defaultButton; // @synthesize defaultButton=_defaultButton;
@property(retain) NSButton *alternativeButton; // @synthesize alternativeButton=_alternativeButton;
@property(retain) NSButton *cancelButton; // @synthesize cancelButton=_cancelButton;
@property(retain) NSTextField *messageTextField; // @synthesize messageTextField=_messageTextField;
@property(retain) NSTextField *titleTextField; // @synthesize titleTextField=_titleTextField;
- (void)defaultButtonPressed:(id)arg1;
- (void)alternativeButtonPressed:(id)arg1;
- (void)cancelButtonPressed:(id)arg1;
- (id)init;

@end

